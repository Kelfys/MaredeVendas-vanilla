-- Papel moderador: abaixo de admin, com acesso de leitura + aprovações
...
-- Lojas: staff lê todas; staff atualiza (aprovações); só admin cria/deleta
...
-- Produtos: staff lê tudo; só dono ou admin altera
...
-- Perfis (moderador precisa ver lojista nas aprovações)
DROP POLICY IF EXISTS "Users can read own profile" ON public.users;
CREATE POLICY "Users can read own profile" ON public.users
  FOR SELECT USING (auth.uid() = id OR public.is_staff());

-- Bloqueia auto-cadastro como moderador
