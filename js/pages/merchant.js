...
function productImageLimitHintHtml(store, products, product = null) {
  ...
  if (!allowed) {
    return `<p class="form-hint form-hint--info">${escapeHtml(planProductImageLimitMessage(store.plan_id))} <a href="${routeHref('/lojista/entrar')}#planos">Ver planos</a></p>`
  }
...
async function renderMerchantPlansPanel(store) {
  ...
  return `
    ...
    <div class="plan-payment-info">
      ...
      <p class="form-hint">Dúvidas sobre limites? <a href="${routeHref('/regras')}">Leia as regras</a>.</p>
    </div>`
}
